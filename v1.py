...
def get_arch_from_dex(apk_path):
    ...
    try:
        with zipfile.ZipFile(apk_path, 'r') as zf:
            ...
            for dex_file in dex_files[:5]:  # 只解析前5个DEX文件避免性能问题
                with zf.open(dex_file) as df:
                    dex_data = df.read(2048*2048)  # 只读前2MB
...
def get_arch_from_xapk_manifest(zip_path):
    """
    方式3：从XAPK的manifest.json解析架构信息（优化版）
    核心优化：
    1. 兼容多版本manifest.json字段（native_codes/abi/architectures等）
    2. 重点解析split_configs/split_apks字段中的架构信息
    3. 适配config.armeabi_v7a、config.arm64_v8a等格式的架构描述
    4. 增强异常处理和日志输出
    :param zip_path: XAPK/APKS文件路径
    :return: 架构集合
    """
    architectures = set()
    # 架构名称映射表（覆盖所有安卓主流架构，兼容config.xxx格式和简写）
    arch_mapping = {
        # 标准架构名
        "arm64-v8a": "arm64-v8a",
    ...
    # 用于从字符串中提取架构特征的正则（匹配config.xxx或纯架构名）
    arch_pattern = re.compile(r'(config\.)?(arm64_v8a|armeabi_v7a|armeabi|x86_64|x86|mips64|mips|universal)', re.IGNORECASE)

    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            if "manifest.json" not in zf.namelist():
                print("XAPK中未找到manifest.json文件")
                return architectures
            ...
            with zf.open("manifest.json") as mf:
                try:
                    ...
                    print(f"manifest.json解析失败（JSON格式错误）：{e}")
                    return architectures

        # 解析逻辑1：原有native_codes字段（基础兼容）
        if "native_codes" in manifest_data:
            for arch in manifest_data["native_codes"]:
                arch_key = arch.strip().lower()
                if arch_key in arch_mapping:
                    architectures.add(arch_mapping[arch_key])

        # 解析逻辑2：根节点abi/architectures字段（兼容其他格式）
        for arch_field in ["abi", "architectures"]:
            if arch_field in manifest_data:
                arch_value = manifest_data[arch_field]
                # 处理字符串类型（如"arm64-v8a,x86"或"config.arm64_v8a"）
                if isinstance(arch_value, str):
                    # 提取架构特征（兼容config.xxx格式）
                    match = arch_pattern.search(arch_value.lower())
                    if match:
                        arch_key = match.group(0).lower()
                        if arch_key in arch_mapping:
                            architectures.add(arch_mapping[arch_key])
                    # 兼容逗号分隔的多架构
        ...
        # 解析逻辑3：核心优化 - split_configs字段（重点）
        if "split_configs" in manifest_data:
            split_configs = manifest_data["split_configs"]
            if isinstance(split_configs, list):
                for config in split_configs:
                    # 提取split_configs中的架构字段（兼容多字段名）
                    for arch_field in ["abi", "architecture", "native_code", "arch", "name", "id"]:
                        if arch_field in config:
                            arch_value = config[arch_field]
                            if isinstance(arch_value, str):
                                # 正则提取架构特征（适配config.xxx格式）
                                match = arch_pattern.search(arch_value.lower())
                                if match:
                                    arch_key = match.group(0).lower()
                                    if arch_key in arch_mapping:
        ...
        # 解析逻辑4：核心优化 - split_apks字段（重点）
        if "split_apks" in manifest_data:
            split_apks = manifest_data["split_apks"]
            if isinstance(split_apks, list):
                for apk in split_apks:
                    # 方式a：直接读取abi/arch/architecture等字段
                    for arch_field in ["abi", "arch", "architecture", "config", "name", "file_name"]:
                        if arch_field in apk:
                            arch_value = apk[arch_field]
                            if isinstance(arch_value, str):
                                # 正则提取架构特征（适配config.xxx格式）
                                match = arch_pattern.search(arch_value.lower())
                                if match:
                                    ...
                                    if arch_key in arch_mapping:
                                        architectures.add(arch_mapping[arch_key])
                                # 兼容纯架构名字符串
                                arch_key = arch_value.strip().lower()
                                if arch_key in arch_mapping:
                                    architectures.add(arch_mapping[arch_key])
                            elif isinstance(arch_value, list):
                                for arch in arch_value:
                                    ...
                                    if arch_key in arch_mapping:
                                        architectures.add(arch_mapping[arch_key])
                    # 方式b：从split_apk的其他嵌套字段提取（如config/abi节点）
                    if "config" in apk and isinstance(apk["config"], dict):
                        config_dict = apk["config"]
                        for arch_field in ["abi", "arch", "architecture"]:
                            if arch_field in config_dict:
                                arch_value = config_dict[arch_field]
                                arch_key = arch_value.strip().lower()
                                if arch_key in arch_mapping:
                                    architectures.add(arch_mapping[arch_key])
        ...
        if is_outer_zip:
            ...
            with zipfile.ZipFile(apk_path, 'r') as zf:
                ...
                if apk_inner_files:
                    ...
                    # 方式5：新增 - 从config.xxx.apk/xapk文件解析（核心新增）
                    arch_config = get_arch_from_config_files(apk_path)
                    if arch_config:
                        all_arch_sets.append(arch_config)
                        detect_methods.append("Config文件")
